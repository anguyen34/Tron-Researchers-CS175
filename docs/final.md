...
Reflecting on our approaches, we realized there were several improvements we could have made to our research process. Perhaps one of the most important improvements was that we could have conducted more games within an epoch when calculating the cumulative rewards and delta rewards to get an average instead of a single value. This would have resulted in our data being more accurate and possibly less volatile. Another mistake we made was when we were calculating the difference in rewards of the modified agent and the 3 unmodified agents, we only normalized the modified agent's reward to remove wins and not the other 3 unmodified agents. In regards to the data we collected, we could have collected other types of data to evaluate on such as win rate. We also acknowledge possible coding mistakes when trying to implement our own agents. In addition, we used several functional approximators that lacked a loss function, and as a result they did not apply as well to reinforcement Q-learning. When observing the performance of parameters within an approximator, it could have been more beneficial to have a general baseline agent that all approximators would train on instead of having them train on agents with the same approximator using default values.
...
The cumulative reward data is completely erratic and inconsistent while the delta reward data is the same but also with a downward trend. Overall this shows that 0.5 is much too high of an epsilon value and that the epsilon parameter itself is a very impactful parameter. To summarize, the most impactful parameters were number of hidden layers, number of nodes per layer, and epsilon while the least impactful was the activation function. For the Neural Agent our main qualitative observation was that it performed the most normally, as in it didn't stick to a single pattern of movements all the time. It acted the closeset to how we had expected a learning AI to play a game of Tron, where it would try new moves.
...
The cumulative reward shows the lowest average performance and a very low plateau compared to the others. The delta reward showed lower values than the other parameter values and a slight downward trend. To summarize, the highest impact parameters are the Support Vector Machine kernel, the MultiLayer Perceptron activation function and hidden layers, and epsilon; while, the lowest impact parameters were found to be the Adaboost number of estimators and Adaboost loss function. The main qualitative observation for our Ensemble agent was that it heavily favored moving in a straight line, but otherwise was unremarkable.
...
While the epsilon value of 0.1 starts with low cumulative reward, its rewards steadily increase and ends with the highest cumulative rewards in the last 2 epochs recorded. For our Random Forest agent, the most impactful parameters were the max depth, max leaf nodes, and epsilon while the least impactful value was the number of estimators. Our main qualitative observation about the Random Forest agent was than in games against other Random Forest agents, was that the Random Forest agents were prone to turning into themselves and then dying. The Random Forest agents often favored turning in contrast to the Ensemble agent, which preferred to go straight. Overall, the Random Forest agent struggled to play the game.
...
The delta reward shows many lows and seems to underperform compared to epsilon of 2. Based on the cumulative reward, it performs decently but not as well as the epsilon value of 0.1. As the epsilon value gets greater, the average cumulative value drops dramatically. Epsilon is an impactful parameter in our Monte Carlo Search Tree agent, but the depth parameter has little impact on the performance of the agent. While no particular depth proved to result in a higher cumulative reward, depth 1 was favored mainly because it required less runtime. The distinctive qualitative observation we saw with Monte Carlo search when against other Monte Carlo agents was that it often spiralled inwards in its corner of the map. This was due to the agent prioritizing maximizing its own score. When playing against other agents the Monte Carlo agent would mainly outlast the other agents and only sometimes spiral into itself.
...
<img src="images/agent_1_competition_reward.png" width="450" /> <img src="images/agent_2_competition_reward.png" width="450" />

<img src="images/agent_3_competition_reward.png" width="450" /> <img src="images/agent_4_competition_reward.png" width="450" />
...
In summary, when evaluating our data, our goal was to find parameters that had an impact on the agents’ learning rate and success. When looking at our neural network approximator, we found the number of hidden layers (1), the number of nodes per layer (64), and the epsilon value (0.1) to be impactful parameters while the activation function didn’t have an impact on the approximator’s learning rate and success. For our Ensemble, we found the SVM Kernel (Poly), the MLP activation function (ReLU), MLP number of hidden layers (50), and the epsilon value (0.1) were the parameters that had an impact on the performance of the agent while the Adaboost number of estimator and Adaboost loss function had no impact on their performance. Our Random Forest Approximator’s impactful parameters were the max depth (None), the max leaf nodes (None), and the epsilon value (0.1) while the number of estimators used appeared to have no impact on the agents. Our agent Monte Carlo search approximator’s impactful parameters were both the depth (1) and the epsilon value (0.1). Finally, for the agents playing against each other our expectations were largely met as the Monte Carlo agent performed the best, but the Neural Network agent showed the most learning.

## References:

- [ColosseumRL](https://colosseumrl.igb.uci.edu/)
- ColosseumRL setup/example from course website for starting our project and the Tron image.
- [SciKit-Learn](https://scikit-learn.org/stable/)
- [Ray](https://www.ray.io/)
- [Gym](https://gym.openai.com/)
- [Tron Light Cycle video](https://www.youtube.com/watch?v=XEp8G2HtDJM)
