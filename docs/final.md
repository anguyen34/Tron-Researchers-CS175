...
Reflecting on our approaches, we realized we could have conducted more games within an epoch when calculating the cumulative rewards and delta rewards to get an average instead. This would have resulted in our data being more accurate and possibly less volatile. Another mistake we made was when we were calculating the difference in rewards of the modified agent and the 3 unmodified agents, we only normalized the modified agent's reward to remove wins and not the other 3 unmodified agents. This resulted in our delta reward being more negative than it should have been when our modified agent lost a game. In regards to the data we collected, we could have collected other types of data to evaluate on such as win rate. We also acknowledge possible coding mistakes when trying to implement our own agents and non differentiable functional approximators that didn’t use code from the Ray Rlib library.  When observing the performance of parameters within an approximator, it could have been more beneficial to have a general baseline agent that all approximators would train on instead of having them train on agents with the same approximator using default values.
